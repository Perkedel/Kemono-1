          ...
          if (Object.keys(post.file).length !== 0) {
            (/\.(gif|jpe?g|png|webp)$/i).test(post.file.path) ? previews += `
              <a class="fileThumb" href="${post.file.path}">
                <img
                  data-src="https://images.weserv.nl/?url=https://kemono.party${post.file.path.replace('https://kemono.party', '')}"
                  src="https://images.weserv.nl/?url=https://kemono.party${post.file.path.replace('https://kemono.party', '')}"
                >
              </a>
              <br>
            ` : attachments += `
          ...
          post.attachments.forEach(attachment => {
            (/\.(gif|jpe?g|png|webp)$/i).test(attachment.path) ? previews += `
              <a class="fileThumb" href="${attachment.path}">
                <img
                  data-src="https://images.weserv.nl/?url=https://kemono.party${attachment.path.replace('https://kemono.party', '')}"
                  src="https://images.weserv.nl/?url=https://kemono.party${attachment.path.replace('https://kemono.party', '')}"
                >
              </a>
              <br>
            ` : attachments += `
