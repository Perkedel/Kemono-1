    ...
    ${props.posts.length ? `
      <div class="views">
        <div class="sidebar" style="margin-right: 20px;">
          ...
          ${Object.keys(props.posts[0].tags).map(namespace => {
            if (!props.posts[0].tags[namespace].length) return '';
            return `
              <div>
                <h6>${capitalize(namespace)}</h6>
                ${props.posts[0].tags[namespace].map(tag => `<li ${namespace !== 'general' ? `class="tag-${namespace}` : ''}"><a href="/posts?tags=${namespace !== 'general' ? `${namespace}%3A` : ''}${tag.replace(/ +/g, '_')}">${tag}</a></li>`).join('')}
              </div>
            `;
          }).join('')}
          <div>
            <h5>Information</h5>
            <li>ID: <a href="/posts?tags=id%3A${props.posts[0].id}" title="The paysite-assigned ID given to this post.">${props.posts[0].id}</a></li>
            <li>User ID: <a href="/posts?tags=user%3A${props.posts[0].user}" title="The paysite-assigned ID given to the artist of this post.">${props.posts[0].user}</a></li>
            <li>Service:
              <a href="/posts?tags=service%3A${props.posts[0].service}" title="The paysite this post was imported from.">
                ${({
                  patreon: 'Patreon',
                  fanbox: 'Pixiv Fanbox',
                  gumroad: 'Gumroad',
          ...
          <div>
            <h5>Related</h5>
            <li><a href="https://translate.yandex.com/?lang=ja-en&text=${encodeURIComponent(props.posts[0].content.replace(/<[^>]+>/g, ''))}">Translate with Yandex</a></li>
            <li><a href="https://translate.google.com/#view=home&op=translate&sl=ja&tl=en&text=${encodeURIComponent(props.posts[0].content.replace(/<[^>]+>/g, ''))}">Translate with Google</a></li>
          </div>
        </div>
          ...
          <div class="search-form" id="edit">
            ...
            >
              ...
              <textarea
                ...
                rows="8"
                autocomplete="off"
              >${stringifyBooruObject(props.posts[0].tags)}</textarea>
              <br>
