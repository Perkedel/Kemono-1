          ...
          if (Object.keys(post.file).length !== 0) {
            (/\.(gif|jpe?g|png|webp)$/i).test(post.file.path) ? previews += `
              <a class="fileThumb" href="${post.file.path}">
                <img
                  data-src="https://images.weserv.nl/?url=https://kemono.party${post.file.path.replace('https://kemono.party', '')}&width=800&output=jpg"
                  src="https://images.weserv.nl/?url=https://kemono.party${post.file.path.replace('https://kemono.party', '')}&width=800&output=jpg"
                >
              </a>
              <br>
            ` : attachments += `
          ...
          post.attachments.forEach(attachment => {
            (/\.(gif|jpe?g|png|webp)$/i).test(attachment.path) ? previews += `
              <a class="fileThumb" href="${attachment.path}">
                <img
                  data-src="https://images.weserv.nl/?url=https://kemono.party${attachment.path.replace('https://kemono.party', '')}&width=800&output=jpg"
                  src="https://images.weserv.nl/?url=https://kemono.party${attachment.path.replace('https://kemono.party', '')}&width=800&output=jpg"
                >
              </a>
              <br>
            ` : attachments += `
