...
(async () => {
  ...
  await posts.createIndex({ user: 1, service: 1 });
  await posts.createIndex({ user: 1, service: 1, published_at: -1 });
  await posts.createIndex({ id: 1, user: 1, service: 1 });
  await posts.createIndex({ id: 1, user: 1, service: 1, published_at: -1 });
  await posts.createIndex({ service: 1 });
  await posts.createIndex({ service: 1, added_at: -1 });
  await posts.createIndex({ added_at: -1 });
  await posts.createIndex({ published_at: -1 });
