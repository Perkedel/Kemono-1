...
async function scraper (key) {
  ...
  const fanboxIndex = await request.get('https://www.pixiv.net/ajax/fanbox/index', requestOptions(key));
...
async function processFanbox (url, key) {
  ...
  Promise.map(postData.items, async (post) => {
    ...
    const postExists = await db.findOne({ id: post.id, service: 'fanbox' });
    if (postExists) return;

    const filesLocation = 'https://kemono.party/fanbox/files';
    const attachmentsLocation = 'https://kemono.party/fanbox/attachments';
    if (post.body.images) {
      await Promise.map(post.body.images, async (image, index) => {
        const fileData = await request.get(unraw(image.originalUrl), fileRequestOptions(key));
    ...
    if (post.body.files) {
      await Promise.map(post.body.files, async (file, index) => {
        const fileData = await request.get(unraw(file.url), fileRequestOptions(key));
        if (index == 0 && !postModel.post_file.name) {
...
async function concatenateArticle (body, key) {
  ...
  await Promise.map(body.blocks, async (block) => {
    if (block.type === 'image') {
      ...
      const fileData = await request.get(unraw(imageInfo.originalUrl), fileRequestOptions(key));
